...
| 欄位 | 說明 |
|------|------|
| `functions.source` | Cloud Functions 程式碼目錄，執行 `firebase deploy --only functions` 時使用 |
| `firestore.indexes` | Firestore 複合索引定義檔路徑（`firestore.indexes.json`） |

> Storage rules 由 Firebase Console 直接管理，不透過 CLI deploy，故無 `storage` 區塊。
